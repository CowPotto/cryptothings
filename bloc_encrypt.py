...
if __name__ == "__main__":
    ...
    output_filename = "result2.txt"    # <<<<< CHANGED: Output file for the results
    target_number = 1062               # The number to subtract from

    print(f"Reading input from: '{input_filename}'")
    print(f"Calculating sum of ASCII values for each item, then subtracting from {target_number}...")
    print(f"Output will be saved to '{output_filename}'") # Updated print statement
    print("-" * 60)
    ...
    if input_list is None:
        ...
        try:
            ...
            with open(output_filename, 'w', encoding='utf-8') as outfile:
                ...
                for item_str in input_list:
                    ...
                    if ascii_sum is not None:
                        ...
                        subtraction_result = target_number - ascii_sum
                        # Format the output string to contain only the subtraction result and a newline
                        output_line = f"{subtraction_result}\n"
            ...
            print("-" * 60)
            print(f"\nCalculation complete. Results saved to '{output_filename}'.") # Updated print statement
            print(f"Processed {len(input_list)} items.")

        except IOError as e:
            print(f"\nError: Could not write to file '{output_filename}'. {e}") # Updated print statement
        except Exception as e:
